...
function AttendeeAvatars({ attendees, getAttendeeList, getInitials }: AttendeeAvatarsProps) {
  ...
  if (Array.isArray(attendees)) {
    if (attendees.length > 0) {
      ...
      } else if (typeof attendees[0] === 'object' && attendees[0]?.name) {
        // Array of objects with name property
        attendeeList = attendees.map((attendee: any) => attendee.name || 'Unknown')
      } else {
        // Fallback to getAttendeeList function
        attendeeList = getAttendeeList(attendees)
      }
    }
  } else {
    // Use the provided function for other formats
    attendeeList = getAttendeeList(attendees)
  }
